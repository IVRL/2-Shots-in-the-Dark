    ...
    def train(self):
        ...
        for i in range(self.args.start_iter, self.args.max_iter):
            ...
            for j, batch_samples in enumerate(self.train_dataloader):
                ...
                if loss.item() > 1:
                    for b in range(clean_img.shape[0]):
                        clean_img_vis = clean_img[b, :-1]
                        clean_img_vis = torch.clamp(clean_img_vis, 0, 1)
        
                        noisy_img_vis = noisy_img[b, :-1]
                        noisy_img_vis = torch.clamp(noisy_img_vis, 0, 1)
        
                        output_vis = output[b, :-1]
                        ...
                        save_name = os.path.join(self.args.vis_save_dir, 'vis_%d_%d_%d.jpg' % (b, i, j))
