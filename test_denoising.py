...
def main():
    parser = argparse.ArgumentParser(description='SID/ELD Denoising Testing')
    ...
    ## Setup log saving path
    args.save_folder = os.path.join(args.save_folder)
    if not os.path.exists(args.save_folder):
        os.makedirs(args.save_folder)
    log_path = os.path.join(args.save_folder, f"test_results_{args.test_dataset}_Ratio{args.ratio}.txt")
    ...
    for img_idx in range(len(input_list)):
        if args.test_dataset == 'SID':
            ...
            print(f" {os.path.basename(in_path)}, ISO: {iso}, Ratio: {ratio} -- PSNR / SSIM: {res['PSNR']:.4f} / {res['SSIM']:.4f}")
            ...
            for scene_id in scenes:
                ...
                for img_id, gt_id in zip(img_ids, gt_ids):
                    ...
                    ssim.append(res['SSIM'])
                    # print(f"Current PSNR: {res['PSNR']}, SSIM: {res['SSIM']}")
                    print(f" {os.path.basename(in_path)}, ISO: {iso_in}, Ratio: {ratio} -- PSNR / SSIM: {res['PSNR']:.4f} / {res['SSIM']:.4f}")
