...
def quantify(image):
    ...
    image = torch.clamp(image, 0, 16383)
    image = torch.round(image).float()
...
def quantify_numpy2(image):
    image = 512 + (16383-512) * image
    image = np.clip(image, 512, 16383)
    image = np.round(image).astype(np.float32)
    image = (image-512) / (16383-512)
    return image


def quantify_numpy3(image):
    image = np.clip(image, 0, 1)
    return image


def extract_darkshading(reference):
    reference_blurred = scipy.ndimage.gaussian_filter(reference, sigma=50)
    return reference_blurred


def local_std_map(img: torch.Tensor, kernel_size: int = 7) -> torch.Tensor:
    """
    Compute local standard deviation for each pixel.
    ...
    Returns:
        std_map: Tensor of shape (C, H, W)
    """
    assert img.ndim == 3, "Input must be (C, H, W)"
    C, H, W = img.shape

    # prepare Gaussian/box kernel
    weight = torch.ones((1, 1, kernel_size, kernel_size), device=img.device, dtype=img.dtype)
    ...
    # local mean
    mean = F.conv2d(x, weight.expand(C, -1, -1, -1), padding=kernel_size // 2, groups=C)

    # local mean of squares
    mean_sq = F.conv2d(x**2, weight.expand(C, -1, -1, -1), padding=kernel_size // 2, groups=C)

    # std = sqrt(E[x^2] - (E[x])^2)
    std = torch.sqrt(torch.clamp(mean_sq - mean**2, min=1e-12))

    return std.squeeze(0)  # (C, H, W)
    

def estimate_prior(img, darkshading, std=None):
    """
    Estimate the fixed prior power spectrum from training data.
    
    Args:
        img: Tensor of shape (4, H, W) containing training samples
        
    Returns:
        P_avg: Average power spectrum of shape (4, H, W)
    """
    EPS = 1e-8
    ...
    power = torch.abs(fft_img) ** 2
    
    return power, darkshading


def sample_from_power(power, darkshading, std=None, device='cpu'):
    """
    Sample correlated noise from the fixed prior.
    
    Args:
        power: Average power spectrum of shape (4, H, W)
        device: Device to place tensors on
        
    Returns:
        samples: Tensor of shape (batch_size, 4, H, W) with correlated noise, in [0,1]
    """
    C, H, W = power.shape
    sample = torch.zeros(C, H, W, device=device)

    norm_factor = torch.sqrt(torch.tensor(H * W, dtype=torch.float32, device=device))
    ...
    for c in range(C):
        ...
        fft_white = fft.fft2(white_noise)
        
        # Scale by the sqrt of the power spectrum and normalize for FFT scaling
        fft_z = fft_white * torch.sqrt(power[c] + 1e-10) / norm_factor
        
        # Convert back to spatial domain
        z = torch.real(fft.ifft2(fft_z))
        
        sample[c] = z

    # Add the darkshading back
    if std is not None:
        sample = sample * std
...
def sample_from_power_numpy(power, darkshading):
    """
    Sample correlated noise from the fixed prior (NumPy version).

    Args:
        power: np.ndarray of shape (C, H, W), average power spectrum
        darkshading: np.ndarray of shape (C, H, W)

    Returns:
        samples: np.ndarray of shape (C, H, W) with correlated noise
    """
    C, H, W = power.shape
    ...
    white_noise = np.random.randn(C, H, W)  # (C,H,W)
    fft_white = np.fft.fft2(white_noise, axes=(-2, -1))
    fft_z = fft_white * np.sqrt(power + 1e-10) / norm_factor
    sample = np.fft.ifft2(fft_z, axes=(-2, -1)).real
    sample += darkshading
