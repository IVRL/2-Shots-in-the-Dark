...
resources_path = '/scratch/students/2023-fall-sp-liying/code/noise_synthesis/LRD/resources/'
...
def get_darkshading_from_average(iso):
    """
    the darkshading is computed by averaging all the darkframes (after normalized to 0-1) of the given ISO in the dataset
    """
    statc_folder = '/scratch/students/2023-fall-sp-liying/code/FlowMatching/notebook/DarkShading'
    # statc_folder = '/scratch/students/2023-fall-sp-liying/code/FlowMatching/notebook/DarkShading_from10real'
    darkshading = np.load(os.path.join(statc_folder, 'ISO%d_mean.npy' % iso))  # shape: (1, 4, H, W)
    std = np.load(os.path.join(statc_folder, 'ISO%d_std.npy' % iso))
    darkshading = darkshading[0].transpose(1,2,0)
    std = std[0].transpose(1,2,0)
    
    return darkshading, std




def get_darkshading_from_singleimage(iso):
    """
    the darkshading is computed by blurring one darkframe (after normalized to 0-1) of the given ISO in the dataset
    """
    statc_folder = '/scratch/students/2023-fall-sp-liying/code/two_shots_in_the_dark/notebook/SampledDarkShadings_BlurSigma50'
    darkshading = np.load(os.path.join(statc_folder, 'ISO%d.npy' % iso))  # shape: (4, H, W)
    
    # statc_folder = '/scratch/students/2023-fall-sp-liying/code/FlowMatching/notebook/DarkShading_1'
    # darkshading = np.load(os.path.join(statc_folder, 'ISO%d_mean.npy' % iso))[0]  # shape: (1, 4, H, W)
    
    darkshading = darkshading.transpose(1,2,0)
    
    return darkshading


    
def get_darkshading_from_pmn(iso):
    ...
    darkshading = pack_np_raw(darkshading)
...
def pack_raw_withdarkshading(raw, iso, ratio):
    ...
    # darkshading, _ = get_darkshading_from_average(iso)
    darkshading = get_darkshading_from_singleimage(iso)
    # darkshading = get_darkshading_from_pmn(iso)
...
def add_darkshading_error(generated_dark_frame, temporal_std_map, n_frames=50):
    """
    Add simulated darkshading estimation error to generated dark frames.
    
    Parameters:
    -----------
    generated_dark_frame : numpy array
        Generated dark frame with shape (4, H, W)
    temporal_std_map : numpy array
        Standard deviation map of temporal noise with shape (4, H, W)
        Calculated from the original dark frames
    n_frames : int
        Number of frames used to compute the original darkshading (default: 50)
    
    Returns:
    --------
    numpy array
        Dark frame with added darkshading error, same shape as input
    """
    # Calculate the standard error of the mean for each pixel
    # SEM = standard_deviation / sqrt(n_samples)
    standard_error_map = temporal_std_map / np.sqrt(n_frames)
    
    # Generate random error for each pixel from normal distribution
    # with mean=0 and standard deviation equal to the standard error
    error_map = np.random.normal(
        loc=0, 
        scale=standard_error_map, 
        size=generated_dark_frame.shape
    )
    
    # Add the error to the generated dark frame
    dark_frame_with_error = generated_dark_frame + error_map
    
    return dark_frame_with_error
